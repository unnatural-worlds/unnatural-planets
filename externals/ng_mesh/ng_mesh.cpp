...
namespace ng_mesh
{
	...
	namespace
	{
		...
		inline void vec4_set(vec4& r, const vec4& x)
		{
			r = x;
		}

		inline void vec4_add(vec4& r, const vec4& x, const vec4& y)
		{
			r = x + y;
		}

		inline void vec4_sub(vec4& r, const vec4& x, const vec4& y)
		{
			r = x - y;
		}

		inline void vec4_scale(vec4& r, const float scale)
		{
			r *= scale;
		}

		inline float vec4_dot(const vec4& x, const vec4& y)
		{
			return dot(x, y).value;
		}

		inline float vec4_length2(const vec4& x)
		{
			return lengthSquared(x).value;
		}
			...
			LinearBuffer<vec4>& collapseNormal)
		{
			...
			for (int i : randomEdges)
			{
				...
				vec4_set(collapsePosition[i], vec4(pos[0], pos[1], pos[2], pos[3]));
